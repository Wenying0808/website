...
export const SciaEngineerFeatureCard = ({img, icon, title, description}: Props) => {
    return(
        <div className="flex flex-col sm:flex-row gap-4 p-3 sm:p-4 md:p-8 bg-background-light-0 dark:bg-background-dark-3 rounded-xl shadow-md hover:scale-105">
            <Image src={img} alt={title} width={400} height={300} className="rounded-lg m-h-[300px]"/>
            <div className="flex flex-col gap-4">
                <div className="flex flex-row gap-2 place-items-center">
                    <div className="flex justify-center place-items-center rounded-full min-w-8 w-8 h-8 bg-primary-light dark:bg-primary-dark text-white dark:text-black">{icon}</div>
